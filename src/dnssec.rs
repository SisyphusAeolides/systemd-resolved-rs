...
use crate::wire::{
    dnskey_key_tag, parse_dnskey, parse_ds, parse_rrsig, ResourceRecord, WireError,
};
use std::error::Error;
use std::fmt;
use std::io;
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedAlgorithm(algorithm) => {
                write!(formatter, "unsupported DNSSEC signature algorithm {algorithm}")
            }
        }
    }
}
...
mod tests {
    ...
    const EXAMPLE_A_SIGNATURE: [u8; 64] = [
        0xe1, 0x81, 0x5f, 0x4f, 0x6d, 0x63, 0x78, 0x64, 0x7f, 0xd4, 0x47, 0x02, 0xbe, 0x7f,
        0x01, 0xd7, 0x80, 0xb4, 0x0e, 0x63, 0x3d, 0x3b, 0xca, 0x9c, 0x18, 0x9b, 0xc7, 0xc1,
        0x96, 0x53, 0xa6, 0xdd, 0xa4, 0xa9, 0x04, 0x9e, 0xd2, 0xf4, 0x16, 0xdd, 0x74, 0xc9,
        0xff, 0x5c, 0xb6, 0x88, 0x94, 0x7e, 0x21, 0x98, 0x55, 0x59, 0x96, 0xc6, 0x47, 0xb6,
        0x9f, 0xda, 0x7e, 0xe2, 0xc7, 0x36, 0xdc, 0x05,
    ];
    ...
    fn native_signature_verification_reports_valid_invalid_and_unsupported() {
        assert!(verify_signature(15, &ED25519_PUBLIC_KEY, &[], &ED25519_EMPTY_SIGNATURE)
    ...
    fn verifies_canonical_ed25519_rrsig_and_rejects_tampering() {
        ...
        assert!(verify_rrsig(&packet, &rrsig, std::slice::from_ref(&answer), &dnskey, now)
            .expect("valid RRSIG"));
